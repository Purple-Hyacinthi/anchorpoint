...
- 会话未晋升期间，pre-step 过滤器剥离 `source.kind` 列在 `suppressedContextSources`
  中的消息（默认 `agent-instructions`、`skill-catalog` 与 `plugin:user-approval`，
  即 Standard 比 Minimal 多出的自动注入 + 宿主审批栈在新会话启动时发出的一次性
  "approval policy changed" 通知；`kind:plugin` 形式精确匹配单个插件）。设为 `[]`
  可关闭上下文过滤；加入其他条目可抑制更多。过滤器自身出错时降级为保留全部消息，
  绝不吞掉上下文。
- 中文桥（`zhBridgeText`）：实测锚定窗口存在语言效应——同样的 Minimal 条件
  （46 字符 persona、2 工具、纯 user 首请求）下，英文任务首段推理涌现
  `We need to …`（headless lab 3/5），中文任务则被翻译导语吃掉窗口
  （`The user is speaking Chinese: …`，GUI 4/4）。默认在第 1 请求检测到汉字时把
  `We need to fulfill the request below. (The user wrote in Chinese — reply in
  Chinese.) Begin your reasoning with a sentence that starts with "We need to"
  describing the work ahead; do not open with a restatement or translation of
  the request.` 合并进首条 user 消息。迭代史：v1 仅框架（1/3，谜题类锚定、编程
  类被无视）→ v2 加反导语条款（编程类仍失败）→ v3 显式指令以 "We need to" 开场
  （headless lab 中文编程题 4/4；GUI 真实使用实测同样锚定，we=13/weNeed=7）。
  同题 A/B（各 4 次）：v2（框架+反导语，不强制开场）**2/4**——两条锚定、两条以
  "The user wants a sort.py…" 开场；v3（硬指令）**4/4**，且两轮 lab 累计 8/8。
  结论：中文编程任务上反导语条款不足以保证开场，显式 "We need to" 指令是必需的。
  设为 `""` 关闭中文桥（恢复纯 Minimal 表面）；英文任务不受影响。
- **live 进程的模块缓存陷阱**：DSH 进程按文件 URL 缓存已加载的预设模块
  （ESM `import()` 缓存），进程不重启时对已加载 `.mjs` 的修改**不会生效**——组合
  代际只跟踪组合 yml 的 stamp。调试期间曾因此让"桥已部署"的判断反复出错
  （GUI 全部失败、lab 全新进程却 4/4）。修改已加载模块时请给文件改名
  （如 `tool-bootstrap-live.mjs`）并更新行引用，或在重启进程后再改。
