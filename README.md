...
## Skills: two ways to keep the anchor stable

The available-skills reminder is the single most disruptive injected context
for the anchor: with the skill catalog present the first request did not
reproduce at all (issue #6, 0/9), without it the same request reproduces at
~81%. The main preset and the `anchored-standard-skills` variant therefore
offer two ways to have skills without breaking the `We need` trajectory:

**Main preset — on-demand skills (`skill_search` / `skill_load`).** The
`dsh-tool-skill` catalog injection is removed entirely. The model discovers
skills with `skill_search` and loads one with `skill_load`, which injects the
skill's instructions for the NEXT request only. The anchor never sees a skill
catalog, at any step. Cost: one tool roundtrip per skill lookup.

**`anchored-standard-skills` variant — deferred catalog.** The official
`dsh-tool-skill` plugin stays mounted, so the `skill` tool, the
`<available_skills>` catalog, and `/skill` gesture injection all work. The
tool-bootstrap strip (the OUTERMOST `agent/pre-step` transform) suppresses the
`skill-catalog` source on request #1 only; the catalog returns from request #2
on, after the anchor has locked the trajectory, and `residentTools: [skill]`
keeps the `skill` tool in the promoted resident set. User-initiated
`skill-invocation` messages are never suppressed.

`skill-catalog-deferred` (preset/skill-catalog-deferred.mjs) is the
preset-owned fallback for the catalog listener itself: `dsh-tool-skill` only
injects when its internal identity check resolves ITS OWN exact `skill`
registration. Measured in the headless lab: with the base layer's `skill`
registration present (dsh-base mounts `dsh-tool-skill` in most profiles), the
official listener never injects from the preset scope — the `skill` tool and
`/skill` gesture still work, but the catalog never appears; disabling the base
row restores the official injection. The fallback performs the byte-identical
catalog injection and de-duplicates against both the decision and the durable
visible surface, so when the official listener works the fallback is a no-op:
one catalog, either way. Suppression stays with the bootstrap strip, so the
catalog remains deferred past the anchor in both cases.

Headless measured (xyit / deepseek-v4-pro, `reasoningEffort=max`, one fresh
session each, same sort.py task): the variant's request #1 is Minimal-exact in
EVERY run — `minimal-complete` persona (46 chars), tools
`[bash, str_replace_editor]`, user message only — and the catalog first
appears at step 2. The "We need …" first line is stochastic per session
(2/4 of our headless runs opened with "We need to do tasks: …"/"We need
respond to user."; the others opened "The user wants me to:"), so single-run
A/B is NOT a measurement — use the lab kit in `experiments/` (METHOD.md) and
the N≥5 modeltest methodology for statistics. The Standard preset on the same
task: 26 tools, harness persona, catalog at step 1, reasoning opens with
"The user wants me to:" (we=0).

Install it as a separate preset id. The variant shares its plugin modules
with the other variants through `../preset/...` relative rows, so the shared
`preset/` directory must also live under its literal name `preset`:
...
**Why "move the skill rows to the bottom of the YAML" does not work.** The
DeepSeek Harness loader applies composition rows CONCURRENTLY —
`cordis-plugin-loader`'s `EntryGroup.update()` runs `Promise.allSettled` over
every row — so YAML row position is not a load-order or listener-order
mechanism, and `dsh-tool-skill`'s catalog listener may register before or
after the bootstrap filter no matter where its row sits. The deterministic
order control is per-listener: the bootstrap's pre-step strip registers with
`prepend: true` (Cordis unshifts it to the front of the listener list), and
waterfall after-next transforms apply in reverse — so the strip is the final
transform and removes what later listeners inject. The same reasoning applies
to the `agent/request` maxTokens cap. "Skill context loads last" is therefore
implemented as a PHASE GATE (`suppressedContextSources` active only while the
session is unpromoted), not as row order.
