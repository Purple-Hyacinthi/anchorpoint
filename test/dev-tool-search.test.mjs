...
test('residentSummary overrides the resident-set sentence (skills variant)', () => {
  const registered = []
  const ctx = { tools: { schemas: () => [], register(tool) { registered.push(tool) } } }
  apply(ctx, { residentSummary: 'This session starts with a minimal resident set: bash, str_replace_editor, dev_tool_search, skill.' })
  const tool = registered.find((t) => t.name === 'dev_tool_search')
  assert.match(tool.description, /dev_tool_search, skill\./)
  assert.doesNotMatch(tool.description, /skill_search/)
})

test('invalid residentSummary values fail at apply time', () => {
  const ctx = { tools: { schemas: () => [], register() {} } }
  assert.throws(() => apply(ctx, { residentSummary: '' }), /residentSummary/)
  assert.throws(() => apply(ctx, { residentSummary: 42 }), /residentSummary/)
})
