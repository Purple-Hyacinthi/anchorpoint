...
const approvalMessage = { id: 'a', content: [], source: { kind: 'plugin', plugin: 'user-approval' } }
const otherPluginMessage = { id: 'o', content: [], source: { kind: 'plugin', plugin: 'some-other-plugin' } }
const hanMessage = { id: 'h', content: [{ type: 'text', text: '写个 sort.py 排三行文本' }], source: { kind: 'user' } }
...
test('bootstrap strip removes the user-approval startup notice', async () => {
  const { listeners } = register()
  const decision = await prestep(listeners['agent/pre-step'], [], [
    userMessage,
    approvalMessage,
  ...
  assert.equal(decision.kind, 'enter')
  // The approval-policy startup notice is stripped; plugin messages from
  // other plugins pass through.
  assert.deepEqual(decision.messages.map((message) => message.id), ['u', 'o'])
})

test('a kind:plugin entry never strips unrelated plugin messages', async () => {
  const { listeners } = register()
  const decision = await prestep(listeners['agent/pre-step'], [], [pluginMessage])
  assert.deepEqual(decision.messages.map((message) => message.id), ['p'])
})

test('the CJK bridge prepends to a Han first user message during bootstrap', async () => {
  const { listeners } = register()
  const decision = await prestep(listeners['agent/pre-step'], [], [hanMessage, instructionMessage])
  const text = decision.messages[0].content[0].text
  assert.ok(text.startsWith('We need to fulfill the request below. (The user wrote in Chinese — reply in Chinese.)'))
  assert.ok(text.includes('写个 sort.py 排三行文本'))
  // The original message object is never mutated.
  assert.equal(hanMessage.content[0].text, '写个 sort.py 排三行文本')
  assert.deepEqual(decision.messages.map((message) => message.id), ['h'])
})

test('the CJK bridge leaves non-Han first user messages untouched', async () => {
  const { listeners } = register()
  const decision = await prestep(listeners['agent/pre-step'], [], [userMessage, instructionMessage])
  assert.equal(decision.messages[0], userMessage)
  assert.equal(decision.messages[0].content[0].text, 'hi')
})

test('a custom zhBridgeText is used verbatim', async () => {
  const { listeners } = register({ ...config, zhBridgeText: 'Task ahead:' })
  const decision = await prestep(listeners['agent/pre-step'], [], [hanMessage])
  assert.ok(decision.messages[0].content[0].text.startsWith('Task ahead:\n\n写个 sort.py'))
})

test('an empty zhBridgeText disables the bridge', async () => {
  const { listeners } = register({ ...config, zhBridgeText: '' })
  const decision = await prestep(listeners['agent/pre-step'], [], [hanMessage])
  assert.equal(decision.messages[0].content[0].text, '写个 sort.py 排三行文本')
})

test('an already-bridged Han message is not bridged twice', async () => {
  const { listeners } = register()
  const bridged = { id: 'b', content: [{ type: 'text', text: 'We need to fulfill the request below. (The user wrote in Chinese — reply in Chinese.) Begin your reasoning with a sentence that starts with "We need to" describing the work ahead; do not open with a restatement or translation of the request.\n\n写个 sort.py' }], source: { kind: 'user' } }
  const decision = await prestep(listeners['agent/pre-step'], [], [bridged])
  assert.equal(decision.messages[0].content[0].text, bridged.content[0].text)
})

test('promoted sessions never receive the CJK bridge', async () => {
  const { listeners } = register()
  const decision = await prestep(listeners['agent/pre-step'], [{ type: 'tool/call' }], [hanMessage])
  assert.equal(decision.messages[0].content[0].text, '写个 sort.py 排三行文本')
})

test('non-string zhBridgeText fails at apply time', () => {
  assert.throws(() => register({ ...config, zhBridgeText: 42 }), /zhBridgeText must be a string/)
})

test('non-boolean bridgeTrace fails at apply time', () => {
  assert.throws(() => register({ ...config, bridgeTrace: 'yes' }), /bridgeTrace must be a boolean/)
})
...
test('unmounted discovery tools are skipped silently in the promoted set', async () => {
  const { listeners, warns } = register()
  const tools = [
...
test('residentTools names stay in the promoted set (the skills seam)', async () => {
  const { listeners } = register({ ...config, residentTools: ['skill'] })
  const tools = [
    { name: 'bash' }, { name: 'str_replace_editor' }, { name: 'dev_tool_search' },
    { name: 'skill' }, { name: 'web_search' },
  ]
  const result = await assemble(listeners['system-prompt/assemble'], [{ type: 'assistant/message' }], tools)
  const names = result.tools.map((tool) => tool.name).sort()
  assert.deepEqual(names, ['bash', 'dev_tool_search', 'skill', 'str_replace_editor'])
  assert.ok(!names.includes('web_search'))
})

test('residentTools does not affect the controlled phase', async () => {
  const { listeners } = register({ ...config, residentTools: ['skill'] })
  const tools = [
    { name: 'bash' }, { name: 'str_replace_editor' }, { name: 'skill' },
  ]
  const result = await assemble(listeners['system-prompt/assemble'], [], tools)
  assert.deepEqual(result.tools.map((tool) => tool.name), ['bash', 'str_replace_editor'])
})

test('an unmounted residentTool is skipped silently', async () => {
  const { listeners, warns } = register({ ...config, residentTools: ['skill'] })
...
test('invalid residentTools values fail at apply time', () => {
  assert.throws(() => register({ ...config, residentTools: [] }), /residentTools/)
  assert.throws(() => register({ ...config, residentTools: ['skill', 42] }), /residentTools/)
})
