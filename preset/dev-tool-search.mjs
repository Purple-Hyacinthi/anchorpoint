...
/** Default resident-set sentence in the tool description (the main preset). */
const DEFAULT_RESIDENT_SUMMARY = 'This session starts with a minimal resident set: bash, str_replace_editor, skill_search, skill_load. Everything else is unlocked on demand through this tool.'

/**
 * One-line description of the resident set shown in the tool description.
 * Compositions whose resident set differs (e.g. the skills variant, which
 * keeps the official `skill` tool instead of skill_search/skill_load)
 * override it via `config.residentSummary`.
 */
function parseResidentSummary(value) {
  if (value === undefined) return DEFAULT_RESIDENT_SUMMARY
  if (typeof value !== 'string' || value.trim().length === 0) {
    throw new TypeError(`${name}: residentSummary must be a non-empty string`)
  }
  return value.trim()
}
...
export function apply(ctx, config) {
  const residentSummary = parseResidentSummary(config?.residentSummary)
  ctx.tools.register({
